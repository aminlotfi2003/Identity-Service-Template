...
public sealed class ApplicationUserTokenRepository : Repository<ApplicationUserToken, Guid>, IApplicationUserTokenRepository
{
    public ApplicationUserTokenRepository(ApplicationDbContext db) : base(db) { }

    public async Task<IReadOnlyList<ApplicationUserToken>> ListByUserAsync(Guid userId, CancellationToken ct = default)
